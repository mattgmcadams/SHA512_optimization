...
int asmgen_parse_syms(struct SymTab **curSyms,
  ...
  while (1) {
    ...
    case TOK_LABEL:
      /* hit a line label */
      if (curSyms != NULL) {
        if (symtab_lookup(curSyms, "$offset", NULL, &offset_offset) != 0) {
          offset_offset=0;
        }
      }
      symtab_record(curSyms, curToken.token, NULL, (offset / 4)+offset_offset);
      break;
...
int asmgen_assemble(struct SymTab **curSyms,
  ...
  unsigned int argCount, fieldNum;
  unsigned int value;
  ...
  while (1) {
	...
	case TOK_INT:
	  ...
	  value = (long long)curToken.value;
